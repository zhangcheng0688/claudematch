...
function checkFounderAuth(request: Request): Response | null {
  ...
  // constant-time compare via shared helper to mitigate timing attacks.
...
export async function handleReconciliationGet(request: Request): Promise<Response> {
  ...
  const { data: summary, error: sumErr } = await supabaseAdmin.rpc("get_funnel_summary", {
...
export const Route = createFileRoute("/api/admin/reconciliation")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => handleReconciliationGet(request),
    },
  },
});
