    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (cachedProfile?.response) {
          ...
          return await saveProfile(supabase, userId, profile_data as never, input, scenario, traceId, request);
        }
        ...
        const perceiveRes = await llmChatEx(
          ...
          { json: true, temperature: 0.85, max_tokens: 3500, label: "call-1-perceive", traceId, timeoutMs: 30_000, deadlineMs: 50_000 },
        );
        ...
        const synthRes = await llmChatEx(
          ...
          { json: true, temperature: 0.85, max_tokens: 1800, label: "call-2-synth", traceId, timeoutMs: 25_000, deadlineMs: 50_000 },
        );
        ...
        return await saveProfile(supabase, userId, profile_data, input, scenario, traceId, request);
      },
    },
  },
});
